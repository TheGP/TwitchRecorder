...
The listener is a small Go web server bound to `127.0.0.1`. It reads completed `.ts` files from the configured folder, plays AAC audio and H.264/AAC video using FFmpeg, and stores watched status and playback positions in the ignored `listener/state.json`. It does not keep converted copies of the recordings. Playback controls include seeking, speed, volume, and fullscreen for video. Click a video to play or pause it, and double-click to toggle fullscreen. Unfinished recordings resume where you left off; positions are saved during playback, on pause, and when the tab closes. The browser also saves the open recording, its current position, playback state, volume, speed, search, filter, and sort in `localStorage`, restoring them after a reload. Browsers may require a click on Play before audio or video can resume. A recording is marked watched when playback reaches the end, which clears its saved position; you can also mark one or selected files watched or unwatched, or mark the whole library watched. Each file has a delete button that asks for confirmation before permanently removing that local copy and its saved status.
