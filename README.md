...
Set `CHAT_OAUTH` to a Twitch user token with the `chat:read` scope. The recorder validates it at startup and records Twitch IRC chat alongside every active Streamlink recording. Keep this token private in `.env`.
...
`OUTPUT_DIR` and `POLL_SECONDS` are optional. Recordings use the channel and server-local start date in their filenames, for example `india-2024-09-09.ts`. If that file already exists, the next recording uses `india-2024-09-09-2.ts`. Streamlink writes to `.ts.part` while recording; chat is written concurrently to the matching `.chat.jsonl.part`. After Streamlink exits, the app finalizes a nonempty media file and its chat sidecar as `.ts` and `.chat.jsonl`. Chat reconnects independently with backoff and never stops the media recording. Each JSON line contains the playback offset, Twitch and receive timestamps, username, display name, text, message ID, badges, color, moderator status, and subscriber status. Capture starts when the recorder detects the stream; Twitch does not provide earlier chat history. Orphaned `.part` files are left for manual recovery after a crash. The app keeps daily sequence numbers in `OUTPUT_DIR/.recording-state` so moving finished files off the server does not reuse their names.

Run a bounded chat-only integration test without starting Streamlink:

```bash
go run ./recorder -chat-test tkkttony -chat-test-duration 60s -chat-test-output /tmp/tkkttony.chat.jsonl
```
...
The transfer ignores `.part` files. For each completed `.ts`, it transfers the matching `.chat.jsonl` first when present, then transfers the media. Every file is downloaded to a temporary name, checked with SHA-256, renamed locally, and removed from the server only if its hash still matches. A mismatch leaves both copies for inspection. Historical recordings without chat sidecars still transfer normally. Run `transfer/transfer-recordings.ps1` manually for an immediate transfer.

## Listener on Windows

The listener is a small Go web server bound to `127.0.0.1`. It reads completed `.ts` files from the configured folder, plays AAC audio and H.264/AAC video using FFmpeg, and stores watched status and playback positions in the ignored `listener/state.json`. It does not keep converted copies of the recordings. Playback controls include seeking, volume, and fullscreen for video. Recordings with a chat sidecar have a Chat button; the panel shows messages up to the current playback position and follows playback or seeking. Click a video to play or pause it, and double-click to toggle fullscreen. Unfinished recordings resume where you left off; positions are saved during playback, on pause, and when the tab closes. The browser also saves the open recording, its current position, playback state, volume, search, filter, and sort in `localStorage`, restoring them after a reload. Browsers may require a click on Play before audio or video can resume. A recording is marked watched when playback reaches the end, which clears its saved position and starts the next unwatched recording in the current order. Playback stops when no unwatched item remains after it. You can also mark one or selected files watched or unwatched, or mark the whole library watched. Each file has a delete button that asks for confirmation before permanently removing that local copy, its saved status, and its matching chat sidecar.
